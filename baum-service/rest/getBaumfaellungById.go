...
func GetBaumfaellungById(c *echo.Context) error {
	// trace span
	...
	log.Info().Str("traceId", traceId).Str("spanId", spanId).Str("path", (*c).Request().URL.Path).Msg("GetBaumfaellungById")

	/* Remove this comment to use session
	session, err := getSession(c)
	if err != nil {
		log.Error().Err(err).Msg("GetBaumfaellungById failed")
	...
	*/
	...
	log.Debug().
		Any("id", id).
		Msg("Received request for GetBaumfaellungById, running business logic")
