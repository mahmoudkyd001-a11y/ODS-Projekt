...
// Antrag löschen
func DeleteBafoegById(c *echo.Context) error {
	// trace span
	...
	log.Info().Str("traceId", traceId).Str("spanId", spanId).Str("path", (*c).Request().URL.Path).Msg("DeleteBafoegById")

	/* Remove this comment to use session
	session, err := getSession(c)
	if err != nil {
		log.Error().Err(err).Msg("DeleteBafoegById failed")
	...
	*/
	...
	log.Debug().
		Any("id", id).
		Msg("Received request for DeleteBafoegById, running business logic")
