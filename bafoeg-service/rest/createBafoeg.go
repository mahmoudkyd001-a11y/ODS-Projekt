...
func CreateBafoeg(c *echo.Context) error {
	// trace span
	...
	log.Info().Str("traceId", traceId).Str("spanId", spanId).Str("path", (*c).Request().URL.Path).Msg("CreateBafoeg")

	/* Remove this comment to use session
	session, err := getSession(c)
	if err != nil {
		log.Error().Err(err).Msg("CreateBafoeg failed")
	...
	*/
	...
	bodyContent := new(entities.Bafoeg)
	if err := middleware.Bind(c, bodyContent); err != nil {
		log.Error().Err(err).Str("traceId", traceId).Str("spanId", spanId).Msg("Failed to bind bodyContent (typeBafoeg) to reqest at CreateBafoeg")
	...
	log.Debug().
		Msg("Received request for CreateBafoeg, running business logic")
